<%--
    Document   : allrefundbills
    Created on : Nov 22, 2019, 2:05:46 PM
...
<%@page import="sampada.DAO.TempRefundBillCorpDAO"%>
...
<html>
    ...
    <body onload="setIframeHeight(this.id);" align="center" style="width:95%;" >
        ...
        <form>
            <fieldset>
                <legend> <h3>Refund Receivable Report</h3></legend>
                ...
                <table id="payableTable" align="center" style="min-height:400px;width:81%;" class="customerTable">
                    ...
                    <tbody>
                        ...
                        <c:forEach items="${queryList1}" var="list">
                            ...
                            <%--<c:if test="${list.billPayableCorp.uniqueId != null  && list.billPayableCorp.weekId>=week_id1 && list.billPayableCorp.weekId<=week_id2 && list.billPayableCorp.billYear==yeari}">--%>
                            <tr>
                                ...
                                <c:if test="${list.pendingAmount!=0}">
                                    <td>${Pending}</td>
                                </c:if>
                                <c:if test="${list.pendingAmount==0}">
                                    <%
                                        BillPayableCorp obj = (BillPayableCorp) pageContext.getAttribute("list");
                                        TempRefundBillCorpDAO trcorpdao = new TempRefundBillCorpDAO();
                                        TempRefundBillCorp obj2 = trcorpdao.getTempRefundByUniqueIDForPayable(obj.getUniqueId());
                                        String refund_date = "Pending DR entries";
                                        
                                        if(obj2!=null) {
                                            BankStatementDAO bkdao = new BankStatementDAO();
                                            BankStatement bkobj2 = bkdao.getBankStatementbyDisburseId(obj2.getSlno(),"Refund");
                                            if(bkobj2!=null)
                                                refund_date = bkobj2.getAmountDate().toString();
                                        }
                                    %>
