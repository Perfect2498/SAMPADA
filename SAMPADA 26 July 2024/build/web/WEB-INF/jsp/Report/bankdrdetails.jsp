<%-- 
    Document   : bankdrdetails
    Created on : 27 Feb, 2020, 11:08:43 AM
    Author     : abt
--%>
...
<html>
    ...
    <body onload="setIframeHeight(this.id);" align="center" style="width:95%;" >
        <h3 align="center" style="color:#003366;">Bank Statement Debited details  From  ${fdate} to  ${ldate} </h3>
        ...
        <form>
            <fieldset>
                ...
                <h3 align="center" style="color:#003366;">uploaded Debited details of Bank Statement</h3>
                <table id="payableTable" align="center" style="min-height:400px;width:81%;" class="customerTable">
                    <thead style="background-color: #0677A1;color: white;height:30px;">
                        <tr>
                            <th>Debited Amount Date</th>
                            <th>Pool Member</th>
                            <!--                        <th>Entity Short Name</th>-->
                            <!--<th>CORPORATE_ID </th>-->

                            <th> Bank Debited Amount</th>
                            <th> Disburse Id</th>
                    ...
                    <tbody>
                        <c:forEach items="${bnkstmtrecon0}" var="list">
                            <tr>

                                <td>${list.amountDate}</td>
                                <td>${list.corporates.corporateName}</td>
                                <td>${list.paidAmount}</td>
                                <c:if test="${list.disburseType == 'Bills'}">
                                    <td>${"B"}${list.disburseId}</td>
                                </c:if>
                                <c:if test="${list.disburseType == 'Refund'}">
                                    <td>${"R"}${list.disburseId}</td>
                                </c:if>
                                <c:if test="${list.disburseType == 'Interest'}">
                                    <td>${"I"}${list.disburseId}</td>
                                </c:if>
                                <c:if test="${list.disburseType == 'PSDF'}">
                                    <td>${"P"}${list.disburseId}</td>
                                </c:if>
                                <c:if test="${list.disburseType == 'Misc'}">
                                    <td>${"M"}${list.disburseId}</td>
                                </c:if>
